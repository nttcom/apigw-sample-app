...
###*
 # @ngdoc function
 # @name nttcomApiGatewayWebappApp.controller:ContractListCtrl
 # @description
 # # ContractListCtrl
...
angular.module 'nttcomApiGatewayWebappApp'
  ...
  [ '$scope', '$routeParams', '$location', '$anchorScroll', 'contractStore', 'menuFactory', 'serviceFactory',
  class ContractListCtrl

    MENU_KEY: 'contracts'

    constructor: (@$scope, @$routeParams, @$location, @$anchorScroll, @contractStore, @menuFactory, @serviceFactory) ->
      ...
      unless @menuFactory[this.MENU_KEY].isServiceDisabled serviceKey
        @loading = true
        @contractStore.request @service, this.onRequestSucceeded, this.onRequestFailed

    onServiceSelected: (evnet, serviceKey) =>
