...
###*
 # @ngdoc service
 # @name nttcomApiGatewayWebappApp.menuFactory
 # @description
 # # menuFactory
  ...
  [ 'menuConstant', (menuConstant) ->
    ...
    class Menu
      constructor: (@key, value) ->
        @name = value.name
        @disabledServiceKeys = value.disabledServiceKeys
      isServiceDisabled: (serviceKey) ->
        (@disabledServiceKeys.indexOf serviceKey) > -1

    # クラスの配列を作成して返却
    menus = {}
    for key in Object.keys(menuConstant)
      menus[key] = new Menu(key, menuConstant[key])
